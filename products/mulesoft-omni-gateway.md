---
type: Product Rename History
title: MuleSoft Omni Gateway
description: "Originally launched as \"MuleSoft Flex Gateway\" (2021). 1 name change since. Now known as \"MuleSoft Omni Gateway\" (as of 2026-05)."
tags: [Integration]
status: stable
sources:
  - id: renameforce
    resource: "https://renameforce.com/renameforce-data.json"
    title: Renameforce dataset
  - id: corroboration
    resource: "https://www.mulesoft.com/platform/omni-gateway-announcement"
    title: MuleSoft - Omni Gateway Announcement
generated: { by: claude-sonnet-5/claude-code, at: "2026-07-26T00:00:00Z" }
verified: { by: claude-haiku-4-5/fact-check-agent, at: "2026-07-26T01:00:00Z" }
vendorOrigin: MuleSoft
---

# Timeline

- **MuleSoft Flex Gateway** (2021 – 2026-05)
- **MuleSoft Omni Gateway** (2026-05 – present)

# Aliases

`Flex Gateway`, `MuleSoft Flex Gateway`, `Omni API Gateway`, `AI Gateway`, `LLM Gateway`, `Federated Gateway`

Sourced from the renameforce.com community dataset.[^renameforce]

[^renameforce]: Renameforce dataset (community-submitted, not independently verified per-entry)

Independently cross-checked by an automated fact-check pass.[^corroboration]

[^corroboration]: Flex Gateway renamed to Omni Gateway May 11, 2026; non-breaking cosmetic change.
